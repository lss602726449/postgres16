...
# Test CREATE TABLE with serial columns
$node_publisher->safe_psql('postgres', "CREATE SEQUENCE serial START 101;");
$node_publisher->safe_psql('postgres', "CREATE TABLE tmp (id serial primary key, data text);");
$node_publisher->safe_psql('postgres', "INSERT INTO tmp VALUES (nextval('serial'), 'one');");
$node_publisher->safe_psql('postgres', "INSERT INTO tmp VALUES (nextval('serial'), 'two');");
$node_publisher->wait_for_catchup('mysub');
$result = $node_subscriber->safe_psql('postgres', "SELECT sum(id) from tmp;");
is($result, qq(203), 'CREATE TABLE with serial column replicated');
$node_publisher->safe_psql('postgres', "DROP TABLE tmp;");
